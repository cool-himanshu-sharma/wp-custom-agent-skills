...
| | |
|---|---|
| **13 commands** | Typed in chat — `/cp-spec`, `/cp-build`, `/cp-security`, `/cp-release` … each starts a defined piece of work |
...
│
├── commands/      ← The 13 /cp-* commands developers type in chat.
...
```
/cp-context                      first time in a plugin — see what you are dealing with
/cp-spec add license expiry notice
/cp-plan
/cp-build
/cp-security                     before anything touching input, output or the database
/cp-release
```

You do not have to use all of them. For a one-line bug fix, `/cp-debug` on its own is
fine — `/cp-triage` exists precisely so the process scales down as well as up.
...
```
L1  Engineering methodology     this repo's cp-* lifecycle skills
...
`cp-security-review` is what turns that fact into a Critical finding with an attack path.
...
|---|---|---|
| `wp-security` | planned | `cp-security-review` + a full vulnerability-class checklist |
| `wp-testing` | planned | `cp-testing` + a minimal wp-env/PHPUnit setup |
| `wp-build-tooling` | planned | `cp-static-analysis` (PHPCS, PHPStan, PHP compatibility) |
...
```bash
node .claude/skills/cp-context-discovery/scripts/plugin_context.mjs ./my-plugin
node .claude/skills/cp-context-discovery/scripts/plugin_context.mjs ./my-plugin --json
```
...
`cp-conventions` ships **empty on purpose**, and reports that it is empty. Nobody
...
`/cp-bootstrap-conventions` against two or more of your real repositories to derive it
from evidence.
...
- [ARCHITECTURE.md](ARCHITECTURE.md) — the four layers, precedence, and which skill hands
  off to which
- [skills/cp-agent-os/references/definition-of-done.md](skills/cp-agent-os/references/definition-of-done.md)
  — the bar every change has to clear, and the honesty rule underneath it
- [skills/cp-implementation/references/plugin-architecture.md](skills/cp-implementation/references/plugin-architecture.md)
  — bootstrap, load order, storage decisions, lifecycle, extensibility
- [skills/cp-implementation/references/coding-standards.md](skills/cp-implementation/references/coding-standards.md)
...
- **"The skill said it was fine" is not a review.** A human reads the diff before it
  merges. Every time.
- **The security and release steps are a floor, not a ceiling.** `/cp-security` catches
