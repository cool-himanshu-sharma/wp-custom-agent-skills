...
- When `cp-conventions` disagrees with what the code now does.
...
for repo in <path1> <path2> <path3>; do
  node .cursor/skills/cp-context-discovery/scripts/plugin_context.mjs "$repo" --json \
...
| Bucket | Rule | Becomes |
|---|---|---|
| **Convention** | Present in **all** scanned repos (or all but one, consistently) | A rule in `cp-conventions` |
...
Write `skills/cp-conventions/references/*.md`, one file per dimension that has
real content. Update `cp-conventions/SKILL.md` so its `description` names the
