...
- After `cp-implementation`, alongside `cp-static-analysis` and `cp-security-review`.
...
**3 · Security.** See `cp-security-review` for the full pass. In review, confirm every
...
Assets enqueued globally? See `cp-performance-review`.
...
finding" table in `cp-security-review` §5 rather than duplicating the judgement here.
...
- **Rubber-stamping your own work.** If you wrote the code, review it as an adversary or
  hand it to the `cp-code-reviewer` persona with fresh context.
