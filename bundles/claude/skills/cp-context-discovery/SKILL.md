...
the **Plugin Context Record** — the factual base every other `cp-*` skill builds on.
...
- When `cp-task-triage` asks whether a change touches a public surface.
...
node .claude/skills/cp-context-discovery/scripts/plugin_context.mjs <plugin-root> --json
```
...
verdict. `superglobal_reads: 31` is an input to `cp-security-review`, not a finding.
