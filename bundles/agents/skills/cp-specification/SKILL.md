...
- When `cp-task-triage` returned **Full** depth.

Skip it for a bug fix that restores intended behavior — that's `cp-debugging`.

## Inputs required

- The request, and who it is for (site admin? editor? another developer? a partner plugin?).
- The Plugin Context Record from `cp-context-discovery`.
