---
name: cp-agent-os
description: "The routing and operating-model skill for WordPress plugin engineering. Use at the start of any WordPress plugin task to pick the right workflow depth and the right skills, and whenever you are unsure which cp-* skill applies. Also use when a WordPress instruction conflicts with a company convention, or when deciding whether a change is done."
...
- When you don't know which `cp-*` skill applies.
...
- **L1 methodology** — this repo's `cp-*` lifecycle skills. *How* to do the work.
- **L2 WordPress knowledge** — the official WordPress skills (`wp-plugin-development`,
  ...
  WordPress is.
- **L3 company conventions** — `cp-conventions`. How *we* build plugins.
...
If L3 is empty because `cp-conventions-bootstrap` hasn't been run, say that rather
...
Load `cp-task-triage`. It classifies the request and picks a **workflow depth**
...
Load `cp-context-discovery` and produce (or reuse) the **Plugin Context Record**. You
...
|---|---|---|
| Deciding what to build | `cp-specification` | — |
| Ordering the work | `cp-planning` | — |
| Writing plugin code | `cp-implementation` | `wp-plugin-development` |
| Admin screens, settings | `cp-implementation` | `wp-plugin-development`, `wpds` |
| REST routes | `cp-implementation` | `wp-rest-api` |
| Blocks, editor UI | `cp-implementation` | `wp-block-development`, `wp-interactivity-api` |
| WP-CLI commands, ops | `cp-implementation` | `wp-wpcli-and-ops` |
| Tests, wp-env, Playwright | `cp-testing` | `wp-playground` |
| PHPCS / PHPStan / lint | `cp-static-analysis` | `wp-phpstan` |
| Nonces, caps, escaping, SQL | `cp-security-review` | — *(fills an upstream gap)* |
| Slow queries, autoload, cron | `cp-performance-review` | `wp-performance` |
| Reviewing a change | `cp-code-review` | — |
| Something is broken | `cp-debugging` | — |
| Versioning, readme, packaging | `cp-release-process` | `wp-plugin-directory-guidelines` |
| Our own house rules | `cp-conventions` | — |
...
3. **Security is not a review stage.** Capability checks, nonces, sanitization, and
   ...
   `cp-security-review` verifies; it does not retrofit.
