...
|---|---|---|
| **Bug** | "broken", "fatal", "not saving", a stack trace, a version that regressed | `cp-debugging` |
| **Feature** | "add", "support", a new screen / route / block / column | `cp-specification` |
| **Refactor** | "clean up", "extract", "rename" — no behavior change intended | `cp-implementation` |
| **Security** | CVE, disclosure report, "unauthenticated", "escalation", audit finding | `cp-security-review` |
| **Performance** | "slow", "timeout", "high TTFB", "memory", a slow-query log | `cp-performance-review` |
| **Compatibility** | new WP/PHP version, conflict with another plugin or theme | `cp-debugging` |
| **Release** | "ship", "tag", "bump", "readme", "deploy to .org" | `cp-release-process` |
| **Support escalation** | a specific site misbehaving; may not be a code defect at all | `cp-debugging` |
...
| Answers | Depth | Path |
|---|---|---|
| All six "no" | **Direct** | `cp-context-discovery` → `cp-implementation` → `cp-static-analysis` → `cp-code-review` |
| Any of 1, 2, 4, 5 | **Standard** | Direct **plus** `cp-testing` (test first) and `cp-security-review` |
| Any of 3 or 6, or the task is a new user-facing surface | **Full** | The complete `/cp-feature` chain, starting at `cp-specification` |
...
Route:   cp-context-discovery → cp-debugging → cp-testing → cp-security-review → cp-code-review
Skipped: cp-specification (no new surface), cp-release-process (not shipping today)
...
- **Treating a support ticket as a bug.** Many escalations are configuration or a
  ...
  (`cp-debugging` covers this) before changing plugin code.
