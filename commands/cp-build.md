...
Invoke the `cp-implementation` skill, with `cp-testing` for anything at Standard depth or
above.

## Modes

- **`/cp-build`** — implement the next pending task, then stop.
- **`/cp-build auto`** — implement every task in the plan after a single approval.
